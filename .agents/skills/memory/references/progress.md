...
- [~] **M0 地基**：`Facet` 声明层骨架 + 内核通用机制（`Node` 注册表 / 词汇元数据 / 路径；`registry` / `signal` /
  `compile` 管线）已落地（Qt-free）。待做：配置 schema（`app.<域>.<page>…` 路径树 / 加载校验）、绑定编译、
  Qt 桥（`Session` 观察者 → Qt 信号）、投影 / 模型、真编译（M1）。
