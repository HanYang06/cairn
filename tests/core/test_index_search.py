...
def test_search_find_update_delete(tmp_path: Path) -> None:
    ...
    note = vault.put_block(Block(body=b"", type="cairn.note"), search_text="apple banana").id
    other = vault.put_block(Block(body=b"", type="cairn.note"), search_text="cherry").id

    found = {str(oid) for oid in vault.search("banana")}
    assert note in found
    assert other not in found

    vault.put_block(Block(id=note, body=b"", type="cairn.note"), search_text="apple durian")
...
def test_rebuild_index_populates_search(tmp_path: Path) -> None:
    vault = _vault(tmp_path)
    oid = vault.put_block(Block(body=b"", type="cairn.note")).id  # 未提供检索文本
