...
状态：**草案 v2.1**（2026-09-22 按内核重构定向回写；桶 / 块 / 目录；删除加密与清单）

> **2026-09-22 方向更新（以 [`kernel-spec.md`](./kernel-spec.md) v1.2 为准）**：
>
> - 存储层是内核**四模块之一**（`Storage`），职责是**对象进 / 出 / 找**；它**不发即时通知、
>   不参与编排、不认识领域语义**。事件是门户的事（`Event` 包，见 kernel-spec §4）。
> - §1 公理「上层一切最终都是块」**收窄**为**存储模块内部的表示**——块是存储的落盘单位，
>   **不是**内核的立论起点，也不再用"是否继承 `Block`"定义对象身份（对象与块分离，进行中）。
> - 参数（保留窗 / 目录版本等）归 **`core/conf/`**（`Conf` = 参数真源）；存储**不再自定义常量**
>   （`RETENTION_MS` 即取自 `core.conf.storage`）。
> - 检索（`search` 表）是**可重建的派生投影**：丢了可 `rebuild_index` 从块重建，不是第二份真源。
> - 字段标注工具（`Attr` / `Data`）**不属存储**，已归 `core/tool/attr.py`（kernel-spec §6.5）。
...
2. **块（Block）**——唯一被存进桶的对象，形如 `{id, checksum, type, body, attrs, ...}`。
