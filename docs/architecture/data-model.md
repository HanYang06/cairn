...
状态：**草案 v0.2**（平行分布存储 / 结构进 DB / 画板模型 / 正文片段列表已定；代码未实现）
...
block =  "一段纯文本"                                  # 默认段落、无样式 → 裸字符串
       | { "kind": "text", "block": "p|h1|h2|h3|li|quote|code",
           "runs": [ run, ... ] }                       # 有块级类型或行内样式
       | { "kind": "ref", "oid": "<OID>" }              # 引用另一个对象（图 / 文件 / 笔记）
       | { "kind": "canvas", ... }                      # 内联画板（§5.3）

run = { "t": "文字", "b": 1, "i": 1, "u": 1, "link": "cairn:<OID>", ... }
```

- **顺序即位置**：块的先后由列表顺序表达，不额外存顺序字段。
- **裸串是默认态**：没有样式的普通段落，直接写字符串，最省。
- **样式一加就拆、一撤就合**：给一段文字加样式，就把该段拆成多个 run；撤销样式则相邻 run 合并回去。
- **规范化（canonical）是硬要求**：同一逻辑内容必须编码成**同一串字节**（服务确定性 CBOR / 签名 / 去重 / 版本哈希）。规则：
  1. 相邻 run「有效格式相同」→ 合并；
  2. 合并后若是**默认格式**（默认段落、无样式）→ 整块写成**裸字符串**。
- **样式绑在元素自己身上**（run 里），**不另开一份按位置对应的样式表**（那会引入对不齐的隐患）。
- **写入口收裸字符串**：`"第一段"` 自动规范成 `{"block":"p","runs":[{"t":"第一段"}]}`（或裸串）。对内统一、对外省事。
...
- 只有**内容（正文块）变化**才 `seq+1` 并归档；标题/标签/`props` 走 `put_meta`，**不产生历史**（`note-model.md` §4.1）。
...
| 术语 | 归属 | 说明 |
|---|---|---|
| 结构库 `structure.db` | §6.4 | 未实现（关系 / 成员 / 标签仍是对象或索引） |
| 关系落 DB（不再是对象） | §4.3 | 未实现 |
| 半加密结构列 + `keyed_hash` 查询 | §6.4 | 未实现 |
| 块列表 + runs + 规范化 | §5.1 | 未实现（现为单 text 片段） |
| 画板（diagram / sketch） | §5.3 | 未实现 |
| `form` 图元（ellipse / rect / polygon） | §5.4 | 未实现 |
| 逻辑图自动布局 / 连线 | §5.3 | 未实现 |
| 多库管理器（`structure.db` / `index.db`） | §6.4 | 未实现 |
